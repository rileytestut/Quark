...
- (void)startScanningForPeers
{
    if (self.peerType != GBALinkPeerTypeClient)
    {
        NSLog(@"Error Scanning: Only Clients may scan for peers.");
        return;
    }
...
{
    NSLog(@"Did start advertising %@", self.currentPeer.name);
...
{
    NSLog(@"Did fail to start advertising %@", self.currentPeer.name);
    NSLog(@"%@", error);
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    ...
    {
        NSLog(@"Problem with central: %ld", (long)central.state);
    }
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    {
        NSLog(@"Updated Peer Name: %@", peripheral.name);
...
- (void)peripheralManagerDidUpdateState:(CBPeripheralManager *)peripheral
{
    ...
    {
        NSLog(@"Adding service %@...", self.linkService);
        [self.peripheralManager addService:self.linkService];
    }
    else
    {
        NSLog(@"Problem with Peripheral Manager: %ld", (long)peripheral.state);
    }
}
